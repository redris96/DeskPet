...
class ReaderEngine {
public:
    ReaderEngine() : isActive(false), isScrollMode(false), scrollSpeed(1), invertDir(false), accumulatedTicks(0), lastScrollSend(0), hasBleStarted(false), kickPending(false), kickTimer(0), isCursorOffset(false) {}
    ...
    void init(lv_obj_t* parent) {
        ...
        lv_obj_add_event_cb(invertBtn, invert_event_wrapper, LV_EVENT_CLICKED, this);
    ...
    void update() {
        ...
        if (currentMillis - lastScrollSend > 15) {
            if (accumulatedTicks != 0 && bleMouse.isConnected() && isScrollMode) {
                ...
                if (multiplier > 10) multiplier = 10; // Cap at 10
                
                base_amount = accumulatedTicks * multiplier;
                
                // Apply User Speed Setting (Scalar)
                // Note: User prompt implies the curve is the main logic, but let's keep the multiplier functional
                // If scrollSpeed is 1 (Low), we use the base. If 3 (Med), we triple it.
                int final_amount = base_amount * scrollSpeed; 
                
                // Apply Direction Invert (Logic from previous steps: Default inverted, User Invert toggles back)
                // Default: -delta. User Invert: +delta.
                final_amount = -final_amount; 
                if (invertDir) final_amount = -final_amount;
        ...
        if (currentMillis - lastCheck > 500) { // Check every 500ms
            ...
            if (connected) {
                ...
                lv_obj_set_style_text_color(headerLabel, lv_color_hex(0x00FF00), 0); // Green
            } else {
                lv_label_set_text(headerLabel, "Waiting for Host...");
                lv_obj_set_style_text_color(headerLabel, lv_color_hex(0xFF5555), 0); // Redish
            }
            
            // Detect State Change to force UI refresh (Fixes "Connecting..." stuck issue)
...
// Old onButtonPress removed
    ...
    void onScroll(int delta) {
        ...
        if (!isScrollMode || !bleMouse.isConnected()) return;
        
        // Momentum Guard: Ignore reverse blips if moving fast
        if (abs(accumulatedTicks) > 2) {
            if ((accumulatedTicks > 0 && delta < 0) || (accumulatedTicks < 0 && delta > 0)) {
                return; // Ignore noise
            }
        }
...
private:
    ...
    void updateVisuals() {
        if (isScrollMode) {
             if (bleMouse.isConnected()) {
                ...
                lv_obj_set_style_text_color(modeLabel, lv_color_hex(0x00FFFF), 0); // Cyan
                lv_label_set_text(modeSubLabel, "Locked: Scroll Active");
             } else {
                // Task 4: Visuals for Delay
                lv_label_set_text(modeLabel, "Connecting...");
                lv_obj_set_style_text_color(modeLabel, lv_color_hex(0xFFA500), 0); // Orange
    ...
    void onButtonPress() {
        ...
        if (isScrollMode) {
             ...
             playTone(1000, 50); delay(50); playTone(1200, 50);
    ...
    void performCursorKick() {
         if (isCursorOffset) return; // Prevent drift by only kicking once per session
    ...
    void toggleSpeed() {
        lastActivityTime = millis();
        if (scrollSpeed == 1) scrollSpeed = 3;
        else if (scrollSpeed == 3) scrollSpeed = 5;
        else scrollSpeed = 1;
    ...
    void toggleInvert() {
        lastActivityTime = millis();
        invertDir = !invertDir;
        lv_obj_set_style_bg_color(invertBtn, invertDir ? lv_color_hex(0x00FF00) : lv_color_hex(0x444444), 0);
    }
